...
Int_t model_sourcetube_geom()
{
	...
	simulation_geom();
	// Build and write to file the same geom, but designed for visualisation (only applicable if composites used)
	visualisation_geom();
	return 0;
}
...
Int_t simulation_geom() {
   ...
   // -------------------------------------
	...
	const char *fileName = "geom/model_sourcetube_geom.root";
...
// glViewer->CurrentCamera().SetExternalCenter(kTRUE);
	...
	// -------------------------------------
	// -- Write out geometry to file
	const char *fileName = "geom/model_sourcetube_vis.root";
